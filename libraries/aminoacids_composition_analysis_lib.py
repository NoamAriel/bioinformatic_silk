...
def _analysis_output_dir(
    ...
    base = root / "data_analysis" / "aa_composition"
...
def filter_records(
    ...
    length_range:
        ...
        whose length is within this range are kept. Uses sequence_length when present.

    length_threshold:
        Optional integer threshold used with `length_mode` (uses sequence_length when present):
    ...
    longest_factor:
        ...
        whose length >= L_max - (L_max / longest_factor). Uses sequence_length when present.

        For example:
        - If L_max = 3000 and longest_factor = 2, only sequences with length
          >= 1500 are kept.
        - If L_max = 3000 and longest_factor = 4, only sequences with length
          >= 2250 are kept.
    ...
    if taxonomy_terms:
        ...
        for rec in filtered:
            ...
            if lineage_lower.intersection(terms_lower):
                tmp.append(rec)
                continue
            # Fall back to organism name matching for species-level filters.
            org_candidates = [
                str(rec.get("organism_name", "") or ""),
                str(rec.get("organism", "") or ""),
            ]
            org_candidates = [c.replace("_", " ").lower() for c in org_candidates if c]
            if any(term in org for term in terms_lower for org in org_candidates):
                tmp.append(rec)
        filtered = tmp
...
def _aggregate_by_species(
    ...
    aggregated: Dict[str, Dict[str, Any]] = defaultdict(
        lambda: {
            ...
            "counts_list": defaultdict(list),
            "fractions": defaultdict(float),
            "fractions_list": defaultdict(list),
            "total_fractions": [],
            "n": 0,
        }
    )
    for rec in records:
        ...
        entry = aggregated[org]
        entry["lengths"].append(rec.get("length", 0) or 0)
        entry["n"] += 1
        rec_counts = rec.get("letter_counts", {}) or {}
        rec_fracs = rec.get("letter_fractions", {}) or {}
        for aa in letters:
            entry["counts"][aa] += float(rec_counts.get(aa, 0))
            entry["counts_list"][aa].append(float(rec_counts.get(aa, 0)))
            entry["fractions"][aa] += float(rec_fracs.get(aa, 0.0))
            entry["fractions_list"][aa].append(float(rec_fracs.get(aa, 0.0)))
        entry["total_fractions"].append(float(rec.get("total_target_fraction", 0.0)))

    out: List[Dict[str, Any]] = []
    for org, payload in aggregated.items():
        n = payload["n"] or 1
        length_sd = statistics.stdev(payload["lengths"]) if len(payload["lengths"]) > 1 else 0.0
        total_frac_sd = (
            statistics.stdev(payload["total_fractions"]) if len(payload["total_fractions"]) > 1 else 0.0
        )
        mean_len = sum(payload["lengths"]) / len(payload["lengths"]) if payload["lengths"] else 0.0
        out.append(
            {
                "organism": org,
                "length": mean_len,
                "length_sd": length_sd,
                "counts": {aa: payload["counts"].get(aa, 0.0) / n for aa in letters},
                "counts_sd": {
                    aa: statistics.stdev(payload["counts_list"][aa])
                    if len(payload["counts_list"][aa]) > 1
                    else 0.0
                    for aa in letters
                },
                "fractions": {aa: payload["fractions"].get(aa, 0.0) / n for aa in letters},
                "fractions_sd": {
                    aa: statistics.stdev(payload["fractions_list"][aa])
                    if len(payload["fractions_list"][aa]) > 1
                    else 0.0
                    for aa in letters
                },
                "total_fraction_mean": sum(payload["total_fractions"]) / n if payload["total_fractions"] else 0.0,
                "total_fraction_sd": total_frac_sd,
            }
        )
    if species_order:
...
def analyze_and_save(
    ...
    json_root = output_root or root
    json_path = write_json(json_root, summary, resolved_json)
    md_path = write_md(json_root, summary, json_path, resolved_md, filter_description=filter_description)
    if plot:
