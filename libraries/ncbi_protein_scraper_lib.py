...
def safe_join(
    ...
    for idx, part in enumerate(parts):
        candidate = path / part
        if len(str(candidate)) >= max_length - tail_reserve:
            remaining = "/".join(parts[idx:])
            short = ("path_" + hex(abs(hash(remaining)) & 0xFFFFFFFF)[2:]).strip("_")
            return path / short
        if len(str(candidate)) >= max_length:
            short = (part[:16] + "_" + hex(abs(hash(part)) & 0xFFFF)[2:]).strip("_")
            candidate = path / short
        if len(str(candidate)) >= max_length:
            short = (part[:8] + "_" + hex(abs(hash(part)) & 0xFFFFFFFF)[2:]).strip("_")
            candidate = path / short
        path = candidate
    return path
...
def _set_rank_meta(node: Dict[str, Any], rank: Optional[str]) -> None:
    if rank and rank.lower() != "no rank" and RANK_META_KEY not in node:
...
def run_ncbi_protein_scraper(
    ...
    target_ranks = ["class","Subclass","Infraclass","Cohort","Superorder","superorder","order", "suborder", "infraorder", "parvorder", "superfamily", 
        ...
        for protein_term in protein_terms:
            ...
            for summaries_chunk in chunked(summaries, BATCH_FETCH):
                ...
                for accession, title, taxid in summaries_chunk:
                    ...
                    species = organism
                    if lineage and species and " " in species:
                        genus_name = species.split()[0].strip()
                        if genus_name and lineage[-1].strip().lower() == genus_name.lower():
                            rank_by_name.setdefault(lineage[-1], "genus")
