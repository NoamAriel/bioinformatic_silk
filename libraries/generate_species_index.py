...
def _iterdir_safe(path: Path):
    try:
        ...
        path_str = str(path)
        if not path.is_absolute() or path_str.startswith("\\\\?\\"):
            return []
        if path_str.startswith("\\\\"):
            long_path = Path("\\\\?\\UNC\\" + path_str.lstrip("\\"))
        else:
            long_path = Path("\\\\?\\" + path_str)
        try:
            return list(long_path.iterdir())
        except OSError:
            return []
...
def count_unique_records(chain_dir: Path) -> int:
    stems = set()
    for item in _iterdir_safe(chain_dir):
        if not item.is_file():
            continue
